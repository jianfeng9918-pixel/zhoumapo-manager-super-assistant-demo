...
## 周麻婆 V9.1 Product Decisions

- 首页继续负责“现在先做什么”，任务页不得再复制同一张主行动卡；任务页固定回答“谁负责、什么节点、哪些待回传、哪些待验收、全部工作在哪里”。
- 任务工作分工以店长、前厅、会员运营、后厨为店长语言入口；营业节点固定表达开店准备、午市复查、晚市准备、高峰现场和收官复盘，来源仍保留在详情与审计记录中。
- `HoldToTalk` 按下即进入说话态，不再向用户暴露350毫秒技术阈值；松开发送、上滑48px取消，轻点展示常用话术，Space/Enter保持可用。正式动作仍须人工确认。
